    ...
    private void loadDirectory() {
        ...
        if (root.isDirectory() && listFiles != null) {
            for (File file : listFiles) {
                if (file.isDirectory()) {
                    fileModelList.add(new FileModel(file.getAbsolutePath(), FileModelType.DIRECTORY, file.getName()));
                } else {
                    final File parent = file.getParentFile();
                    if (parent == null) {
                        continue;
                    }
                    fileModelList.add(new FileModel(file.getAbsolutePath(), parent.getAbsolutePath(), FileModelType.FILE, file.getName()));
                }
            }
        } else {
            fileModelList.add(new FileModel(root.getAbsolutePath(), FileModelType.DIRECTORY, root.getName()));
        }
